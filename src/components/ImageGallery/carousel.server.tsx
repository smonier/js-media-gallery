...
import CarouselClient from "./carousel.island.client";
...
export default jahiaComponent<ImageGalleryProps>(
  ...
  (props: ImageGalleryProps, { renderContext, currentNode }) => {
    ...
    console.log("[ImageGallery Carousel] Component rendering - Props:", {
      hasFolder: !!folder,
    ...
    let imageNodes: any[] = [];
    if (folder) {
      console.log("[ImageGallery Carousel] Folder mode - trying to access folder");
      try {
        ...
        if (folderNode) {
          console.log("[ImageGallery Carousel] Folder node path: " + folderNode.getPath());
          ...
          console.log(
            "[ImageGallery Carousel] Found " + imageNodes.length + " image nodes in folder",
          );
        }
      } catch (error) {
        console.error("[ImageGallery Carousel] Error getting folder images: " + String(error));
      }
    } else if (imagesList) {
      console.log(
        "[ImageGallery Carousel] ImagesList mode - " + (imagesList?.length || 0) + " images",
      );
